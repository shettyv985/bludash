...
function StatBox({
  ...
  return (
    ...
    >
      <p className={`text-[10px] font-semibold tracking-widest uppercase ${dark ? "text-white/25" : "text-slate-400"}`}>
        {label}
      </p>
      ...
      return (
        ...
        >
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
            ...
            <span className="text-white/60 text-[11px] bg-black/50 px-3 py-1 rounded-full">
    ...
    return (
      <div className={`w-full rounded-t-2xl flex flex-col items-center justify-center gap-2 py-12 ${dark ? "bg-black/40" : "bg-slate-100"}`}>
        <svg
          ...
          stroke="currentColor"
          strokeWidth="1"
          className={dark ? "text-white/15" : "text-slate-300"}
        >
        ...
        <div className="flex flex-col gap-5 p-6">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-[15px] font-semibold leading-snug pr-8 ${dark ? "text-white" : "text-slate-900"}`}>
                {ad.name}
              </p>
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full tracking-widest uppercase ${statusColor}`}>
                  {ad.status}
                </span>
          ...
          <div className={`rounded-xl px-4 py-3 border flex flex-col gap-2 ${dark ? "bg-white/[0.02] border-white/[0.05]" : "bg-slate-50 border-black/[0.07]"}`}>
            <div className="flex items-center gap-2">
              ...
              <p className={`text-[10px] font-semibold tracking-widest uppercase w-16 flex-shrink-0 ${dark ? "text-white/25" : "text-slate-400"}`}>
                Campaign
              </p>
              <p className={`text-[12px] font-medium truncate ${dark ? "text-white/70" : "text-slate-700"}`}>
                {ad.campaignName}
              </p>
            </div>
              ...
              <div className={`flex items-center gap-4 pt-1.5 border-t ${dark ? "border-white/[0.05]" : "border-black/[0.06]"}`}>
                {ad.dailyBudget && (
                  <div className="flex items-center gap-1.5">
                    <p className={`text-[10px] ${dark ? "text-white/25" : "text-slate-400"}`}>Daily</p>
                    <p className={`text-[12px] font-semibold ${dark ? "text-amber-400" : "text-amber-600"}`}>
                      {fmtMoney(ad.dailyBudget)}
                    </p>
                  </div>
                )}
                ...
                {ad.lifetimeBudget && (
                  <div className="flex items-center gap-1.5">
                    <p className={`text-[10px] ${dark ? "text-white/25" : "text-slate-400"}`}>Lifetime</p>
                    <p className={`text-[12px] font-semibold ${dark ? "text-amber-400" : "text-amber-600"}`}>
                      {fmtMoney(ad.lifetimeBudget)}
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="flex flex-col gap-2">
            <p className={`text-[10px] font-bold tracking-widest uppercase ${dark ? "text-white/25" : "text-slate-400"}`}>
              Delivery
            </p>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <StatBox
                label="CTR"
                value={fmtPct(ins.ctr)}
                dark={dark}
                accent={getCTRAccent(ins.ctr, dark)}
                sub={ins.ctr >= 1.5 ? "Good" : ins.ctr >= 0.8 ? "Average" : "Below avg"}
              />
              <StatBox
                label="CPC"
          ...
          {hasEngagement && (
            <div className="flex flex-col gap-2">
              <p className={`text-[10px] font-bold tracking-widest uppercase ${dark ? "text-blue-400/70" : "text-blue-600"}`}>
                Engagement
              </p>
              <div className="grid grid-cols-3 gap-2">
                {ins.likes > 0 && (
                  <StatBox label="Likes" value={fmt(ins.likes)} dark={dark} accent={dark ? "text-blue-400" : "text-blue-600"} />
                )}
                {ins.comments > 0 && (
                  <StatBox label="Comments" value={fmt(ins.comments)} dark={dark} accent={dark ? "text-blue-400" : "text-blue-600"} />
                )}
                {ins.shares > 0 && (
                  <StatBox label="Shares" value={fmt(ins.shares)} dark={dark} accent={dark ? "text-blue-400" : "text-blue-600"} />
                )}
                {ins.videoViews > 0 && (
                  <StatBox label="Video Views" value={fmt(ins.videoViews)} dark={dark} accent={dark ? "text-blue-400" : "text-blue-600"} />
                )}
              </div>
            </div>
          )}
            ...
            <div className={`w-full h-1.5 rounded-full ${dark ? "bg-white/[0.06]" : "bg-slate-200"}`}>
              <div
                className={`h-1.5 rounded-full transition-all ${
                  ins.ctr >= 1.5 ? "bg-emerald-500" : ins.ctr >= 0.8 ? "bg-yellow-500" : "bg-red-500"
                }`}
