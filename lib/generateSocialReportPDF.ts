...
// Takes the Manus JSON analysis + raw social payload → creates a second Manus task
...
export async function generateSocialReportPDF(
  ...
  const { ok: createOk, data: createData } = await safeFetch("/api/social-manus-html", {
    method: "POST",
  ...
  while (polls < MAX_POLLS) {
    ...
    const brief: string  = data?.brief  ?? "Manus is building your social HTML report…";
